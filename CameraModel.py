...
def field(l,a,x,y):
    ...
    wavelength l from K three-dimensional positions x[:] with K
    ...
    return np.sum((a[np.newaxis,:] / dist) * np.exp(2. * np.pi * 1j * dist / l), axis=1) 
...
def parabolic_mirror(D, f, xres):
    ...
    spacing between points in the x-y plane to xres.
    '''
...
def plot_lens_and_intensity(x,y,I):
    ...
    plt.subplot(2,2,4)
    plt.gray()
    plt.imshow(np.log(I.reshape((Ny, Ny))), vmin=-6., vmax=4.)
    plt.axis('equal')
    return None
...
def main():
    ...
    xp = parabolic_mirror(D, f, 0.5 * lam)
    x = distort_mirror(xp)
    K = len(x)
